...
hmm_11 <- hmmfit(traindayform$x, startmodel, mstep = mstep.norm,maxit = 200, tol=1e-02)

#print resulting HMM parameters
summary(hmm)
plot(hmm$loglik, type="b", ylab="log-likelihood", xlab="Iteration")

#yhat1 <- predict (hmm_11,traindayform$x)
#yhat2 <- predict (hmm_11,test1form$x)

#plot(yhat1)
#plot(yhat2)
hmm_11$loglik[length(hmm_11$loglik)]